...
import static com.gohool.repo.CameraKit.Constants.FACING_BACK;
import static com.gohool.repo.CameraKit.Constants.FACING_FRONT;
...
import Base.CameraImpl;
import Base.CameraViewLayout;
import Base.EventDispatcher;
import Base.PreviewImpl;
import Base.SurfaceViewPreview;
import Types.CaptureMethod;
import Types.Facing;
import Types.Flash;
import Types.Focus;
import Types.Permissions;
import Types.VideoQuality;
import api16.Camera1;
import events.CameraKitEventCallback;
import events.CameraKitEventListener;
import events.CameraKitImage;
import events.CameraKitVideo;
import util.AspectRatio;
import util.DisplayOrientationDetector;
import util.PostProcessor;
import vision.CameraKitTextDetect;
import vision.GooglePlayServicesUnavailableException;
import vision.TextProcessor;
...
public class CameraView extends CameraViewLayout {
    ...
    public CameraView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        mCameraImpl = new Camera1 ( mEventDispatcher, mPreviewImpl ) {
            ...
            @Override
            public
            void captureImage(ImageCapturedCallback callback) {
            ...
            @Override
            public
            android.util.Size getPreviewResolution() {
                return null;
            }
        ...
        if (mAdjustViewBounds) {
            ...
            if (previewSize != null) {
                if (getLayoutParams().width == LayoutParams.WRAP_CONTENT) {
                    ...
                    float ratio = (float) height / (float) mPreviewImpl.getHeight ();
                    int width = (int) (mPreviewImpl.getWidth () * ratio);
                    super.onMeasure(
                    ...
                    float ratio = (float) width / (float) mPreviewImpl.getWidth ();
                    int height = (int) (mPreviewImpl.getHeight () * ratio);
                    super.onMeasure(
                            widthMeasureSpec,
    ...
    public
    android.util.Size getCaptureSize() {
